...
module.exports = {
  ...
  async execute(interaction, { utils, db: data }) {
    ...
    if (has.tickets_1.categorys_open) {
      await db.set("categorys_open", categorys.id);
      return interaction.editReply("**تم تعديل كتجوري الفتح بنجاح.**");
    } else {
      await db.set("categorys_open", categorys.id);
      return interaction.editReply("**تم تعيين كتجوري الفتح بنجاح.**");
    }
  },
};
